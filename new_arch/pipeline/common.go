...
func ConstructPostConsensusMessage(t types.PartialSigMsgType) func(pipeline *Pipeline, objects ...interface{}) (error, []interface{}) {
	return func(pipeline *Pipeline, objects ...interface{}) (error, []interface{}) {
...
func SignBeaconObject(pipeline *Pipeline, objects ...interface{}) (error, []interface{}) {
	//cd := objects[0].(*types.ConsensusData)
	// sign with domain
...
func QBFTProcessMessage(pipeline *Pipeline, objects ...interface{}) (error, []interface{}) {
	prevDecided := false
	if pipeline.Runner.GetQBFT().Decided() {
		prevDecided = true
	}

	msgToBroadcast, err := pipeline.Runner.GetQBFT().ProcessMessage(objects[0].(*qbft.SignedMessage))
	if err != nil {
		return err, nil
	}

	if msgToBroadcast != nil {
		err, _ := Broadcast(p2p.SSVConsensusMsgType)(pipeline, msgToBroadcast)
		if err != nil {
			return err, nil
		}
	}

	if !pipeline.Runner.GetQBFT().Decided() || prevDecided {
		return nil, []interface{}{Stop}
	}

	return nil, []interface{}{pipeline.Runner.GetQBFT().DecidedValue}
}
...
func AddPostConsensusMessage(pipeline *Pipeline, objects ...interface{}) (error, []interface{}) {
	return nil, objects
}

// ValidatePartialSignatureForSlot validates a provided post consensus message
func ValidatePartialSignatureForSlot(pipeline *Pipeline, objects ...interface{}) (error, []interface{}) {
...
func VerifyExpectedRoots(pipeline *Pipeline, objects ...interface{}) (error, []interface{}) {
